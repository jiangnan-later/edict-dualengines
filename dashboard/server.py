...
def get_morning_config_payload():
    """Return morning config, injecting display_language if missing."""
    migrate_notification_config()
    payload = read_json(DATA / 'morning_brief_config.json', _DEFAULT_MORNING_CONFIG)
    if not isinstance(payload, dict):
        payload = dict(_DEFAULT_MORNING_CONFIG)
    payload.setdefault('display_language', 'zh-CN')
    payload.setdefault('categories', _DEFAULT_MORNING_CONFIG['categories'])
    payload.setdefault('keywords', _DEFAULT_MORNING_CONFIG['keywords'])
    payload.setdefault('custom_feeds', _DEFAULT_MORNING_CONFIG['custom_feeds'])
    return payload
...
KNOWN_HERMES_MODELS = [
    {'id': 'mimo-v2.5-pro', 'label': 'mimo-v2.5-pro', 'provider': 'custom:token-plan-sgp.xiaomimimo.com'},
...
def _read_hermes_skills(limit=60):
    """Return a compact list of installed Hermes skills for dashboard display."""
...
def _read_hermes_model_config():
    """Read Hermes default model/provider from ~/.hermes/config.yaml.

    Keep this dependency-light: prefer PyYAML if available, otherwise parse the
    simple top-level model.default/model.provider shape Hermes writes.
    """
    cfg_path = pathlib.Path.home() / '.hermes' / 'config.yaml'
    default_model = os.getenv('HERMES_MODEL', '') or 'hermes-default'
    provider = os.getenv('HERMES_PROVIDER', '') or 'Hermes'
    try:
    ...
    try:
        ...
        default_model = model_cfg.get('default') or model_cfg.get('name') or default_model
        provider = model_cfg.get('provider') or provider
        return str(default_model), str(provider)
    except Exception:
        in_model = False
        for raw in text.splitlines():
            ...
            if stripped.startswith('default:'):
                default_model = stripped.split(':', 1)[1].strip().strip('"\'') or default_model
            elif stripped.startswith('provider:'):
                provider = stripped.split(':', 1)[1].strip().strip('"\'') or provider
        return default_model, provider


def _default_agent_config_payload():
    runtime_cfg = _get_agent_runtime_config()
    if runtime_cfg.runtime == 'hermes':
        default_model, provider = _read_hermes_model_config()
        known_models = list(KNOWN_HERMES_MODELS)
        if default_model not in {m['id'] for m in known_models}:
            known_models.insert(0, {'id': default_model, 'label': default_model, 'provider': provider})
        hermes_skills = _read_hermes_skills()
        agents = []
    ...
    return {'agents': [], 'knownModels': [], 'dispatchChannel': ''}


def get_agent_config_payload():
    payload = read_json(DATA / 'agent_config.json', {})
    if isinstance(payload, dict) and isinstance(payload.get('agents'), list):
        if payload.get('agents'):
            return payload
    return _default_agent_config_payload()


def _state_agent_id(task):
    ...
    if state == 'Menxia':
        return 'menxia'
    if state in ('Assigned', 'Review'):
        return 'shangshu'
    org = (task or {}).get('org', '')
    org_map = {'户部': 'hubu', '礼部': 'libu', '兵部': 'bingbu', '刑部': 'xingbu', '工部': 'gongbu', '吏部': 'libu_hr', '钦天监': 'zaochao'}
    return org_map.get(org, '')


def _default_officials_stats_payload():
    agent_cfg = get_agent_config_payload()
    tasks = load_tasks()
    officials = []
    done_count = sum(1 for t in tasks if (t or {}).get('state') == 'Done')
    for idx, ag in enumerate(agent_cfg.get('agents', []), start=1):
        active_tasks = [t for t in tasks if _state_agent_id(t) == ag.get('id') and (t or {}).get('state') not in ('Done', 'Cancelled')]
        participated = [{'id': t.get('id', ''), 'title': t.get('title', ''), 'state': t.get('state', '')} for t in active_tasks]
        officials.append({
            'id': ag.get('id', ''),
            'label': ag.get('label', ''),
            'emoji': ag.get('emoji', '🏛️'),
            'role': ag.get('role', ''),
            'rank': next((d.get('rank', '') for d in _AGENT_DEPTS if d['id'] == ag.get('id')), ''),
            'model': ag.get('model', agent_cfg.get('defaultModel', '')),
            'model_short': str(ag.get('model', agent_cfg.get('defaultModel', ''))).split('/')[-1],
            'tokens_in': 0,
            'tokens_out': 0,
            'cache_read': 0,
            'cache_write': 0,
            ...
            'flow_participations': len(participated),
            'merit_score': len(active_tasks) * 10,
            'merit_rank': idx,
            'last_active': now_iso() if active_tasks else '',
            'heartbeat': {'status': 'active', 'label': '🟢 活跃'} if active_tasks else {'status': 'idle', 'label': '⚪ 待命'},
            'participated_edicts': participated,
        })
    officials.sort(key=lambda o: o['merit_score'], reverse=True)
    for idx, off in enumerate(officials, start=1):
        off['merit_rank'] = idx
    return {
        'officials': officials,
        'totals': {'tasks_done': done_count, 'cost_cny': 0},
        'top_official': officials[0]['role'] if officials else '—',
        'runtime': agent_cfg.get('runtime', 'hermes'),
        'generatedAt': now_iso(),
    }


def get_officials_stats_payload():
    payload = read_json(DATA / 'officials_stats.json', {})
    if isinstance(payload, dict) and isinstance(payload.get('officials'), list):
        if payload.get('officials'):
            return payload
    return _default_officials_stats_payload()
...
def _check_gateway_alive():
    """检测当前 Agent Runtime 是否可用。"""
    runtime_cfg = _get_agent_runtime_config()
    if runtime_cfg.runtime == 'hermes':
        return resolve_runtime_binary(runtime_cfg) is not None
...
def get_agents_status():
    ...
    for dept in _AGENT_DEPTS:
        ...
        if runtime_cfg.runtime == 'hermes':
            has_workspace = True
            last_ts = last_ts or int(datetime.datetime.now().timestamp() * 1000)
            process_alive = gateway_alive
    ...
    if runtime_cfg.runtime == 'hermes':
        gateway_status = '🟢 Hermes 可用' if gateway_alive else '🔴 Hermes CLI 未找到'
    else:
        gateway_status = '🟢 运行中' if gateway_probe else ('🟡 进程在但无响应' if gateway_alive else '🔴 未启动')
...
def get_live_status_payload():
    """Return live status, normalizing health for the selected agent runtime."""
    task_data_dir = get_task_data_dir()
    payload = read_json(task_data_dir / 'live_status.json', {})
    if not isinstance(payload, dict):
        payload = {}

    tasks = load_tasks()
    payload['tasks'] = tasks if isinstance(tasks, list) else []
    payload.setdefault('syncStatus', {})

    runtime_cfg = _get_agent_runtime_config()
    runtime_available = _check_gateway_alive()
    raw_health = payload.get('health')
    health = dict(raw_health) if isinstance(raw_health, dict) else {}
    if runtime_cfg.runtime == 'hermes':
        status_label = 'Hermes 模式在线' if runtime_available else 'Hermes CLI 未找到'
    else:
        status_label = str(health.get('statusLabel', ''))
    health.update({
        'runtime': runtime_cfg.runtime,
        'syncOk': bool(runtime_available),
        'statusLabel': status_label,
        'missingFieldCount': health.get('missingFieldCount', 0),
    })
    payload['health'] = health
    payload['generatedAt'] = now_iso()
    payload['taskSource'] = 'tasks_source.json'
    payload.setdefault('officials', [])
    payload.setdefault('history', [])
    payload.setdefault('metrics', {
        'officialCount': 0,
        'todayDone': 0,
        'totalDone': 0,
        'inProgress': sum(1 for t in payload['tasks'] if (t or {}).get('state') not in ('Done', 'Cancelled')),
        'blocked': sum(1 for t in payload['tasks'] if (t or {}).get('state') == 'Blocked'),
    })
    return payload
...
def _get_agent_runtime_config():
    """Return the dashboard dispatch runtime configuration."""
    env = dict(os.environ)
    env.setdefault('EDICT_AGENT_RUNTIME', 'hermes')
    return AgentRuntimeConfig.from_env(env)


def _resolve_openclaw_bin():
    """Backward-compatible resolver for tests and OpenClaw-only callers."""
    configured = os.environ.get('OPENCLAW_BIN', '').strip() or 'openclaw'
    return resolve_runtime_binary(AgentRuntimeConfig(runtime='openclaw', openclaw_bin=configured))


def _resolve_runtime_bin(config=None):
    """Return the configured agent runtime binary path, if available."""
    return resolve_runtime_binary(config or _get_agent_runtime_config())
...
def scan_and_recover_dispatch_gaps(threshold_sec=60):
    """Find tasks whose current state has not been dispatched yet.

    Agent-side `kanban_update.py state` writes JSON directly; it cannot call the
    dashboard's in-process `dispatch_for_state`. This scanner bridges that gap.
    """
    pending = []
    actions = []
    ...
    def _scan(tasks):
        for task in tasks:
            ...
            if not agent_id:
                continue
            sched = _ensure_scheduler(task)
            if sched.get('lastDispatchState') == state and sched.get('lastDispatchAgent') == agent_id:
                status = sched.get('lastDispatchStatus')
                if status in ('queued', 'success'):
                    continue
                last_dispatch = _parse_iso(sched.get('lastDispatchAt'))
                if last_dispatch:
                    cooldown_sec = max(30, int(threshold_sec or 60))
                    age = max(0, int((datetime.datetime.now().astimezone() - last_dispatch.astimezone()).total_seconds()))
                    if age < cooldown_sec:
                        continue
            now_dt = datetime.datetime.now().astimezone()
            updated_at = _parse_iso(task.get('updatedAt') or sched.get('lastProgressAt'))
            if updated_at:
                age = max(0, int((now_dt - updated_at.astimezone()).total_seconds()))
                if age < int(threshold_sec or 0):
                    continue
            now_text = now_iso()
            sched['lastDispatchAt'] = now_text
            sched['lastProgressAt'] = now_text
            sched['stallSince'] = None
            sched['lastDispatchStatus'] = 'queued'
            sched['lastDispatchAgent'] = agent_id
            sched['lastDispatchState'] = state
            sched['lastDispatchTrigger'] = 'state-dispatch-gap'
            _scheduler_add_flow(task, f'检测到状态 {state} 尚未派发，自动续派至 {agent_id}', to=_STATE_LABELS.get(state, state))
            pending.append((task_id, state))
            actions.append({'taskId': task_id, 'state': state, 'agent': agent_id, 'action': 'dispatch-gap'})
        return tasks
    ...
    for task_id, state in pending:
        task = next((t for t in tasks if t.get('id') == task_id), None)
        if task:
            dispatch_for_state(task_id, task, state, trigger='state-dispatch-gap')
    return {'ok': True, 'count': len(actions), 'actions': actions, 'checkedAt': now_iso()}
...
def handle_scheduler_tick(threshold_sec=600):
    """Run all scheduler maintenance passes for one dashboard tick."""
    dispatch_gap = scan_and_recover_dispatch_gaps(threshold_sec=0)
    stalled = handle_scheduler_scan(threshold_sec)
    actions = []
    for result in (dispatch_gap, stalled):
        if isinstance(result, dict):
            actions.extend(result.get('actions') or [])
    return {
        'ok': bool(dispatch_gap.get('ok') and stalled.get('ok')),
        'thresholdSec': max(60, int(threshold_sec or 600)),
...
def dispatch_for_state(task_id, task, new_state, trigger='state-transition'):
    ...
    def _do_dispatch():
        try:
            ...
            runtime_cfg = _get_agent_runtime_config()
            runtime_bin = _resolve_runtime_bin(runtime_cfg)
            if not runtime_bin:
                err = missing_binary_error(runtime_cfg)
                log.warning(f'⚠️ {task_id} 自动派发异常: {err}')
                _update_task_scheduler(task_id, lambda t, s: (
                    s.update({
                        'lastDispatchAt': now_iso(),
                        'lastDispatchStatus': runtime_cfg.missing_status,
                        'lastDispatchAgent': agent_id,
                        'lastDispatchTrigger': trigger,
                        'lastDispatchError': err,
                    }),
                    _scheduler_add_flow(t, f'派发异常：{runtime_cfg.display_name} CLI 未找到（{trigger}）', to=t.get('org', ''))
                ))
                return
            runtime_cfg = AgentRuntimeConfig(
                runtime=runtime_cfg.runtime,
                openclaw_bin=runtime_bin if runtime_cfg.runtime == 'openclaw' else runtime_cfg.openclaw_bin,
                hermes_bin=runtime_bin if runtime_cfg.runtime == 'hermes' else runtime_cfg.hermes_bin,
                hermes_profile_mode=runtime_cfg.hermes_profile_mode,
                hermes_toolsets=runtime_cfg.hermes_toolsets,
                hermes_max_turns=runtime_cfg.hermes_max_turns,
                project_dir=runtime_cfg.project_dir,
            )
            cmd = build_dispatch_command(runtime_cfg, agent_id, msg, timeout=300, channel=_channel)
            max_retries = 2
            err = ''
            ...
            runtime_cfg = _get_agent_runtime_config()
            err = f'{runtime_cfg.display_name} CLI 未找到：{e}'
            log.warning(f'⚠️ {task_id} 自动派发异常: {err}')
            _update_task_scheduler(task_id, lambda t, s: (
                s.update({
                    'lastDispatchAt': now_iso(),
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p in ('', '/dashboard', '/dashboard.html'):
            ...
            self.send_json(get_live_status_payload())
        elif p == '/api/agent-config':
            self.send_json(get_agent_config_payload())
    ...
    def do_POST(self):
        ...
        if p == '/api/scheduler-scan':
            ...
            try:
                result = handle_scheduler_tick(threshold_sec)
    ...
    def _periodic_scheduler_scan():
        while True:
            try:
                import time as _time
                _time.sleep(120)
                result = handle_scheduler_tick(threshold_sec=180)
