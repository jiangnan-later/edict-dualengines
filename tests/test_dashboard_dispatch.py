...
def test_dispatch_records_missing_openclaw_cli(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv('OPENCLAW_BIN', 'definitely-not-openclaw')
...
def test_dispatch_records_missing_hermes_cli(monkeypatch, tmp_path):
    """Missing Hermes CLI should be reported with Hermes-specific status."""
    import server as srv
    ...
    monkeypatch.setenv('HERMES_BIN', 'definitely-not-hermes')
    monkeypatch.setattr(srv, 'DATA', data_dir)
    monkeypatch.setattr(srv, '_ACTIVE_TASK_DATA_DIR', data_dir)
    monkeypatch.setattr(srv, '_check_gateway_alive', lambda: True)
    monkeypatch.setattr(
    ...
    updated = json.loads(tasks_path.read_text(encoding='utf-8'))[0]
    sched = updated['_scheduler']
    assert sched['lastDispatchStatus'] == 'hermes-missing'
    assert 'Hermes CLI 未找到' in sched['lastDispatchError']
    assert any('Hermes CLI 未找到' in item['remark'] for item in updated['flow_log'])
