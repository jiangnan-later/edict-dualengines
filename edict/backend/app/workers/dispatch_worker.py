...
class DispatchWorker:
    ...
    async def _call_openclaw(
        ...
        """异步调用配置的 Agent Runtime CLI — 在线程池中执行，带富上下文注入。"""
        settings = get_settings()
        runtime_cfg = AgentRuntimeConfig(
            ...
            hermes_profile_mode=settings.hermes_profile_mode,
            hermes_toolsets=settings.hermes_toolsets,
            hermes_max_turns=settings.hermes_max_turns,
            project_dir=settings.openclaw_project_dir,
        ).normalized()
