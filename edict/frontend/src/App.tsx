...
export default function App() {
  ...
  const syncOk = typeof health?.syncOk === 'boolean' ? health.syncOk : sync?.ok;
  const syncLabel = health?.statusLabel || (syncOk ? '同步正常' : syncOk === false ? '服务器未启动' : '连接中…');
      ...
      <div className="hdr">
        ...
        <div className="hdr-r">
          <span className={`chip ${syncOk ? 'ok' : syncOk === false ? 'err' : ''}`}>
            {syncOk ? `✅ ${syncLabel}` : syncOk === false ? `❌ ${syncLabel}` : `⏳ ${syncLabel}`}
          </span>
